...
const CheckoutModal = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const dMethods = db.getAll('delivery_methods');
  ...
  const handleCheckout = (e) => {
    ...
    if (appliedCoupon) {
      if (appliedCoupon.discountType === 'percentage') {
        currentDiscount = currentSubtotal * (appliedCoupon.discountValue / 100);
      } else {
    ...
    // Update Customer (create or increment orders)
    const existingCust = db.getAll('customers').find(c => c.email === customerInfo.email);
    if (existingCust) {
      db.update('customers', existingCust.id, { 
    ...
    // Deduct Stock
    cart.forEach(item => {
      const dbProduct = db.getById('products', item.product.id);
      if (dbProduct) {
        db.update('products', dbProduct.id, { stock: dbProduct.stock - item.quantity });
      }
    });
  ...
  if (appliedCoupon) {
    if (appliedCoupon.discountType === 'percentage') {
      cartDiscountAmount = cartSubtotal * (appliedCoupon.discountValue / 100);
    } else {
  ...
  if (customerInfo.municipality !== 'San Pedro Sula') {
    filteredDeliveryMethods = availableDeliveryMethods.filter(
      m => !m.name.toLowerCase().includes('pick up') && !m.name.toLowerCase().includes('pickup')
    );
  }

  const isPickUp = !!selectedDelivery && (
    selectedDelivery.name.toLowerCase().includes('pick up') ||
    selectedDelivery.name.toLowerCase().includes('pickup')
  );
                    ...
                    <div style={{ display: 'flex', gap: '16px', marginBottom: '16px' }}>
                      <div className="form-group" style={{ flex: 1, marginBottom: 0 }}>
                        <select
                          ...
                          style={{ width: '100%', background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', color: customerInfo.department ? 'var(--text-primary)' : 'var(--text-secondary)', padding: '12px', borderRadius: '12px', fontFamily: 'inherit', outline: 'none', cursor: 'pointer' }}
                        >
                          <option value="" disabled>Departamento</option>
                          {Object.keys(hondurasLocations).sort().map(dept => (
                            <option key={dept} value={dept} style={{color: 'var(--text-primary)'}}>{dept}</option>
                          ))}
                        </select>
                      </div>
                      <div className="form-group" style={{ flex: 1, marginBottom: 0 }}>
                        <select
                          ...
                          onChange={e => {
                            ...
                            if (newMuni !== 'San Pedro Sula') {
                              ...
                              if (currentSelected && (currentSelected.name.toLowerCase().includes('pick up') || currentSelected.name.toLowerCase().includes('pickup'))) {
                                setDeliveryMethodId('');
                              }
                            }
                          }}
                          disabled={!customerInfo.department}
                          style={{ width: '100%', background: customerInfo.department ? 'var(--bg-secondary)' : 'var(--bg-primary)', border: '1px solid var(--border-color)', color: customerInfo.municipality ? 'var(--text-primary)' : 'var(--text-secondary)', padding: '12px', borderRadius: '12px', fontFamily: 'inherit', outline: 'none', cursor: customerInfo.department ? 'pointer' : 'not-allowed' }}
                        >
                          <option value="" disabled>Municipio</option>
                          {customerInfo.department && hondurasLocations[customerInfo.department].sort().map(muni => (
                            <option key={muni} value={muni} style={{color: 'var(--text-primary)'}}>{muni}</option>
                          ))}
                        </select>
                      </div>
                    </div>
                    {!isPickUp && (
                      <div className="form-group" style={{ marginBottom: '16px' }}>
                        <textarea 
                          ...
                          style={{ width: '100%', background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', color: 'var(--text-primary)', padding: '12px', borderRadius: '12px', fontFamily: 'inherit', resize: 'vertical', outline: 'none' }}
                        />
                      </div>
                    )}
                    {filteredDeliveryMethods.length > 0 && (
                      <div className="form-group" style={{ marginBottom: '16px' }}>
                        <select 
                          ...
                          style={{ width: '100%', background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', color: deliveryMethodId ? 'var(--text-primary)' : 'var(--text-secondary)', padding: '12px', borderRadius: '12px', fontFamily: 'inherit', outline: 'none', cursor: 'pointer', textOverflow: 'ellipsis' }}
                        >
                    ...
                    {availableMethods.length > 0 && (
                      <div className="form-group" style={{ marginBottom: '16px' }}>
                        <select
                          ...
                          style={{ width: '100%', background: 'var(--bg-secondary)', border: '1px solid var(--border-color)', color: 'var(--text-primary)', padding: '12px', borderRadius: '12px', fontFamily: 'inherit', outline: 'none', cursor: 'pointer', textOverflow: 'ellipsis' }}
                        >
              ...
              {cart.length > 0 && (
                <div className="checkout-sidebar">
                    ...
                    {appliedCoupon && (
                      <div className="summary-row discount" style={{ color: 'var(--danger)', margin: '4px 0' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          ...
                          <button type="button" className="btn-icon danger" style={{ padding: '2px', width: '20px', height: '20px', color: 'var(--danger)', background: 'rgba(231, 76, 60, 0.1)' }} onClick={() => setAppliedCoupon(null)} title="Quitar cupón">
                            <X size={12} />
                          </button>
                        </div>
                    ...
                    <div className="coupon-section" style={{ marginBottom: '20px', padding: '15px', background: 'var(--bg-tertiary)', borderRadius: '12px', border: '1px dashed var(--border-color)' }}>
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <input
                          ...
                          style={{ flex: 1, padding: '10px', borderRadius: '8px', border: '1px solid var(--border-color)', outline: 'none', background: 'var(--bg-secondary)', color: 'var(--text-primary)', textTransform: 'uppercase' }}
                        />
                  ...
                  <button
                    ...
                    style={{ fontSize: '1.1rem', padding: '16px', borderRadius: '12px', marginTop: '8px' }}
                  >
                    Confirmar Pedido (L. {cartTotalAmount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })})
                  </button>
                </div>
              )}
            </>
