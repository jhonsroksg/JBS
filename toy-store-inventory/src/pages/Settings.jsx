...
const Settings = () => {
  ...
  const loadData = () => {
    setMethods(db.getAll('payment_methods'));
    setDeliveryMethods(db.getAll('delivery_methods'));
    setCoupons(db.getAll('coupons'));
  ...
  const handleAdd = (e) => {
    ...
    db.insert('payment_methods', { name: newMethod.trim() });
    setNewMethod('');
    loadData();
  };
  ...
  const handleSaveEdit = () => {
    if (editingName.trim()) {
      db.update('payment_methods', editingId, { name: editingName.trim() });
    }
    setEditingId(null);
    setEditingName('');
    loadData();
  ...
  const handleAddDelivery = (e) => {
    ...
    db.insert('delivery_methods', { 
      name: newDeliveryName.trim(), 
      cost: Number(newDeliveryCost) || 0 
    });
    setNewDeliveryName('');
    setNewDeliveryCost('');
    loadData();
  ...
  const handleSaveEditDelivery = () => {
    if (editingDeliveryName.trim()) {
      db.update('delivery_methods', editingDeliveryId, { 
        name: editingDeliveryName.trim(), 
        cost: Number(editingDeliveryCost) || 0 
      });
    }
  ...
  const handleSaveEditCoupon = () => {
    if (editingCouponCode.trim()) {
      db.update('coupons', editingCouponId, { 
  ...
  const handleToggleCoupon = (id, currentStatus) => {
    db.update('coupons', id, { isActive: !currentStatus });
    loadData();
  };
  ...
  return (
    <div className="settings-page">
      ...
      <div className="glass-panel" style={{padding: '30px', marginBottom: '24px'}}>
        <h2 style={{fontSize: '1.4rem', marginBottom: '20px', borderBottom: '1px solid var(--border-color)', paddingBottom: '12px'}}>
        ...
        <form onSubmit={handleSaveStoreInfo} style={{display: 'flex', flexDirection: 'column', gap: '16px'}}>
          ...
          <div>
            ...
            <textarea 
              value={storeInfo.welcomeMessage}
              onChange={e => setStoreInfo({...storeInfo, welcomeMessage: e.target.value})}
      ...
      <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px'}}>
        ...
        <div className="glass-panel" style={{padding: '30px'}}>
          ...
          <form onSubmit={handleAdd} style={{display: 'flex', gap: '12px', marginBottom: '24px'}}>
            <input 
              type="text" 
              placeholder="Añadir pago (Ej. Zelle...)" 
              value={newMethod}
              onChange={(e) => setNewMethod(e.target.value)}
              style={{flex: 1, ...inputStyle}}
            />
            <button type="submit" className="btn-primary" style={{borderRadius: '12px', padding: '0 16px'}}>
              <Plus size={20} />
            </button>
          </form>
                ...
                <div key={method.id} style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px', background: 'var(--bg-tertiary)', borderRadius: '12px', border: '1px solid var(--border-color)'}}>
                  {editingId === method.id ? (
                    <input 
                      ...
                      style={{flex: 1, marginRight: '16px', padding: '8px', borderRadius: '8px', border: '1px solid var(--accent-primary)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', outline: 'none', fontFamily: 'inherit'}}
                    />
                  ) : (
                ...
                <div key={method.id} style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px', background: 'var(--bg-tertiary)', borderRadius: '12px', border: '1px solid var(--border-color)'}}>
                  {editingDeliveryId === method.id ? (
                    <div style={{display: 'flex', gap: '8px', flex: 1, marginRight: '16px'}}>
                      <input 
                        type="text" 
                        value={editingDeliveryName} 
                        onChange={e => setEditingDeliveryName(e.target.value)}
                        placeholder="Nombre"
                        style={{flex: 2, padding: '8px', borderRadius: '8px', border: '1px solid var(--accent-primary)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', outline: 'none', fontFamily: 'inherit'}}
                      />
                      <input 
                        type="number" 
                  ...
                  {editingCouponId === coupon.id ? (
                    <div style={{display: 'flex', gap: '8px', flex: 1, marginRight: '16px'}}>
                      <input 
                        ...
                        onChange={e => setEditingCouponCode(e.target.value.toUpperCase())}
                        style={{flex: 2, padding: '8px', borderRadius: '8px', border: '1px solid var(--accent-primary)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', outline: 'none', fontFamily: 'inherit'}}
                      />
                      <select 
                        value={editingCouponType}
                        onChange={(e) => setEditingCouponType(e.target.value)}
                        style={{flex: 1, padding: '8px', borderRadius: '8px', border: '1px solid var(--accent-primary)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', outline: 'none', fontFamily: 'inherit'}}
                      >
                        <option value="percentage">Porcentaje (%)</option>
                        <option value="fixed">Monto Fijo (L.)</option>
